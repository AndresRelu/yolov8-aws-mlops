...
def copy_training_artifact(run_dir, model_dir, filename):
    ...
    if not os.path.exists(source_path):
        print(f"⚠️ Alerta: No se encontró {filename} en {source_path}.")
        return None

    os.makedirs(artifact_dir, exist_ok=True)
    shutil.copy(source_path, destination_path)
    print(f"✅ Artefacto de evaluación empaquetado: {destination_path}")
...
def main():
    ...
    # Ultralytics guarda los resultados en args.output_data_dir/kitti_train
    training_run_dir = os.path.join(args.output_data_dir, "kitti_train")
    local_best_path = os.path.join(training_run_dir, "weights", "best.pt")
    ...
    print("📈 Empaquetando resultados visuales y tabulares del entrenamiento...")
    copied_artifacts = [
        copy_training_artifact(training_run_dir, args.model_dir, "results.png"),
        copy_training_artifact(training_run_dir, args.model_dir, "results.csv"),
    ]
    copied_artifacts = [artifact for artifact in copied_artifacts if artifact]

    if copied_artifacts:
        print("✅ Resultados listos dentro de model.tar.gz:")
        for artifact in copied_artifacts:
            print(f" - {artifact}")
    else:
        print("⚠️ No se encontraron results.png/results.csv para empaquetar.")
